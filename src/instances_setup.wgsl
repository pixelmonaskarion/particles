...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    textureStore(particles, vec3u(global_id.x + global_workgroup_size.x * 2, global_id.yz), vec4f(f32(global_id.x)/f32(global_workgroup_size.x), f32(global_id.y)/f32(global_workgroup_size.y), f32(global_id.z)/f32(global_workgroup_size.z), 0.01));
}
